...
class KNInteractorTest : public celeritas_test::InteractorHostTestBase
{
  ...
  protected:
    ...
    void sanity_check(const Interaction& interaction) const
    {
        ...
        EXPECT_EQ(celeritas::Action::scatter, interaction.action);
  ...
  protected:
    celeritas::KNInteractorPointers pointers_;
};
...
TEST_F(KNInteractorTest, ten_mev)
{
    // Reserve 4 secondaries
    this->resize_secondaries(4);

    // Create the interactor
    KNInteractor  interact(pointers_,
                          this->particle_track(),
                          this->direction(),
    ...
    // Next sample should fail because we're out of secondary buffer space
    {
        ...
        EXPECT_EQ(celeritas::Action::failure, result.action);
    }
}

TEST_F(KNInteractorTest, stress_test)
{
    RandomEngine& rng_engine = this->rng();
